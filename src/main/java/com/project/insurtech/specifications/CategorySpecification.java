...
public class CategorySpecification {
    ...
    public static Specification<Category> findByProviderIdAndIsDeleted(Long providerId, IsDeletedEnum isDeleted) {
        return (root, query, criteriaBuilder) -> {
            Predicate providerIdPredicate = criteriaBuilder.equal(root.get("user").get("id"), providerId);
            Predicate deletedPredicate = criteriaBuilder.equal(root.get("isDeleted"), isDeleted.getValue());
            return criteriaBuilder.and(providerIdPredicate, deletedPredicate);
        };
    }
